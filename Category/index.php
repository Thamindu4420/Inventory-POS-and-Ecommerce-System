...
if (isset($_GET['delete'])) {
  ...
  $sqlDelete = "DELETE FROM category WHERE category_id = '$categoryIdToDelete'";
  $resultDelete = mysqli_query($conn, $sqlDelete);

  if ($resultDelete) {
    // Query successful, update the total category count in the session
    session_start();
    $sqlCountCategory = "SELECT COUNT(*) AS total_category FROM category";
    $resultCountCategory = mysqli_query($conn, $sqlCountCategory);
    $data = mysqli_fetch_assoc($resultCountCategory);
    $_SESSION['total_category'] = $data['total_category'];
  }
}

// Fetch all items from the 'category' table
...
// Store the total category count in the session variable
$_SESSION['total_category'] = $totalCategory;
?>
...
<head>
  ...
  function deleteCategory(categoryId) {
    if (confirm('Are you sure you want to delete this category?')) {
      // Redirect back to the category page after deleting
      window.location.href = 'http://localhost/Kumara%20Stores%20Inventory_system/Category/?delete=' + categoryId;
    }
  }
</script>
...
<div class="content">
  <!-- "Manage Category" and "Home > Category" texts -->
  <div class="texts">
    <div class="manage-category-text">Manage Category</div>
    <div class="breadcrumb-text">Home &gt; Category</div>
  </div>
  <div>
  <!-- Add Category Button -->
  <a href="http://localhost/Kumara%20Stores%20Inventory_system/Add_Category/">
...
// Attach the searchCategory function to the "input" event of the search input
document.getElementById("searchInput").addEventListener("input", searchCategory);
</script>


<!-- Table -->
<table class="category-table" style="transform: translateY(40px);">
  <thead>
    <tr>
...
<tr>
      ...
      <td>
        <a href="http://localhost/Kumara%20Stores%20Inventory_system/Add_Category/?edit=<?php echo $categoryId; ?>">
        ...
        <button class="delete-button" onclick="deleteCategory(<?php echo $categoryId; ?>)">
...
<div class="sidebar" style="transform: translateY(98px);">
  <ul class="menu">
    ...
    <li><img src="https://www.nicepng.com/png/full/204-2049036_download-3-cube-icon.png" alt="Category"><a href="">Category</a></li>
